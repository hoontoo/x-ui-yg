...
install_x-ui() {
    ...
    if  [ $# == 0 ] ;then
        wget -N --no-check-certificate -O /usr/local/x-ui-linux-${arch}.tar.gz https://raw.githubusercontents.com/hoontoo/x-ui-yg/main/x-ui-linux-${arch}.tar.gz
        if [[ $? -ne 0 ]]; then
        ...
        last_version=$1
        url="https://raw.githubusercontents.com/hoontoo/x-ui-yg/main/x-ui-linux-${arch}.tar.gz"
    ...
    wget --no-check-certificate -O /usr/bin/x-ui https://raw.githubusercontents.com/hoontoo/x-ui-yg/main/x-ui.sh
...
red "x-ui安装失败，请查看日志，运行 x-ui log，反馈到https://github.com/hoontoo/x-ui-yg/issues"
fi
    sleep 1
    echo -e ""
    echo -e "$int"
